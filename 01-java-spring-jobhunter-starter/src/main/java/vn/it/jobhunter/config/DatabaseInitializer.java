...
public class DatabaseInitializer implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        if (countPermissions == 0) {
            ...
            arr.add(new Permission("Get resume with pagination", "/api/v1/resumes", "GET", "RESUMES"));
            arr.add(new Permission("Get resume with pagination", "/api/v1/resumes/by-user", "POST", "RESUMES"));

            arr.add(new Permission("Create a role", "/api/v1/roles", "POST", "ROLES"));
            arr.add(new Permission("Update a role", "/api/v1/roles", "PUT", "ROLES"));
            arr.add(new Permission("Delete a role", "/api/v1/roles/{id}", " DELETE", "ROLES"));
            arr.add(new Permission("Get a role by id", "/api/v1/roles/{id}", " GET", "ROLES"));
            arr.add(new Permission("Get role with pagination", "/api/v1/roles", "GET", "ROLES"));

            arr.add(new Permission("Create a user", "/api/v1/users", "POST", "USERS"));
            arr.add(new Permission("Update a user", "/api/v1/users", "PUT", "USERS"));
            arr.add(new Permission("Delete a user", "/api/v1/users/{id}", " DELETE", "USERS"));
            arr.add(new Permission("Get a user by id", "/api/v1/users/{id}", " GET", "USERS"));
            arr.add(new Permission("Get user with pagination", "/api/v1/users", "GET", "USERS"));
            ...
            arr.add(new Permission("Get subscriber with pagination", "/api/v1/subscribers", "GET", "SUBSCRIBERS"));

            arr.add(new Permission("Dowload file", "/api/v1/files", "GET", "FILES"));
        ...
        if (countPermissions > 0 && countRoles > 0 && countUsers > 0) {
            System.out.println(">>>> SKIP INIT DATABAASE ");
        } else {
            System.out.println(">>>> END INIT DATABAASE ");
        }
